...
public class FilmDbStorage extends BaseRepository<Film> implements FilmStorage, LikeStorage {
    ...
    public boolean deleteFilmById(Long filmId) {
        ...
        logger.info("Удаление пользовательских связей для фильма с ID: {}", filmId);
        int deletedRelationsCount = jdbc.update("DELETE FROM film_user WHERE film_id = ?", filmId);
        logger.info("Удалено {} пользовательских связей для фильма с ID: {}", deletedRelationsCount, filmId);
    ...
    public void clearFilm() {
        ...
        logger.info("Удаление всех пользовательских связей из базы данных");
        jdbc.update("DELETE FROM film_user");
    ...
    public Set<Long> deleteLike(Long filmId, Long userId) {
        if (isLikeExists(filmId, userId)) {
            update(DELETE_LIKE_FILM, filmId, userId);
    ...
    private Mpa getMpaByFilmId(Long filmId) {
        return jdbc.queryForObject("SELECT m.id, m.name FROM film_mpa fm JOIN mpa m ON fm.mpa_id = m.id " +
                "WHERE fm.film_id = ?", new Object[]{filmId}, new MpaRowMapper());
    }
